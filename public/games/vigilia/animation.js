...
export function updatePlayerAnimation(player, dt, moving) {
  ...
  const runSpeed = Math.max(0.35, Math.min(1, Math.abs(player.vx) / 252));
  const phase =
    state === 'run'
  ...
  player.animation = {
    state,
    phase,
    time: changed ? 0 : previous.time + dt,
    frame:
      state === 'run'
        ? Math.floor(phase * RUN_FRAMES)
        : state === 'dash'
          ? POSES.apex
          : POSES[state],
  };
...
export function characterAtlas(id) {
  return ATLAS[id] || ATLAS.kaia;
}
...
export function enemyFrame(enemy) {
  ...
  if (enemy.active && enemy.shootCd >= 0 && enemy.shootCd < 0.36) {
    return 7 + smoothstep(1 - enemy.shootCd / 0.36) * 2;
  }
