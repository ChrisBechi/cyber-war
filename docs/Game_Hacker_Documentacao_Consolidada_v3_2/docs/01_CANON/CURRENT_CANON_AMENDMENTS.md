...
## 3. A Garota
A garota é uma pessoa comum e determinada. **Ela não entende hacking, não aprende hacking e não vira investigadora técnica.** No começo ela nem conhece o nickname. Após a invasão, busca entender de forma leiga como invasões de redes sociais acontecem, pergunta em comunidades e conversa com pessoas. Com o tempo, extrai de Gregory informações sutis sobre o amigo que ela não conhece — confiabilidade, proximidade, onde vive e contexto — até reunir fragmentos suficientes para chegar a um nickname/identidade candidata. Para transformar suspeita em prova técnica, precisa de alguém capaz: NULL.
...
A Garota continua sendo completamente leiga em tecnologia. Entre as Sessões 1 e 5, sua determinação deixa rastros narrativos: primeiro perguntas gerais sobre invasão de redes sociais, recuperação de conta, logs, dispositivos e como seria possível identificar quem fez aquilo; depois conversas com pessoas e perguntas cada vez melhores. **Ela não começa pesquisando `[NICKNAME]`, porque ainda não conhece esse nome.** Somente mais tarde os fragmentos obtidos de Gregory permitem chegar ao nickname/identidade candidata. O protagonista pode encontrar rastros dessas perguntas sem saber que pertencem a ela.
...
No **Prólogo / First Boot**, o foco de terminal é filesystem e navegação básica: `pwd`, `ls`, `cd`, `mkdir`, `rm`, `cp`, `mv`, `cat`, `clear`, além de `wget` quando houver contexto de download por terminal. `ping`, `ipconfig` e `ifconfig` **não entram no prólogo**, pois ainda não existe uma necessidade narrativa de troubleshooting de rede.

Na missão de **Wi-Fi da Sessão 1**, `ping` introduz conectividade/resposta do alvo; `ipconfig` aparece como hábito trazido do Windows e `ifconfig` como equivalente no ambiente Linux/Kali-like. A troca acidental de `ipconfig` por `ifconfig` pode funcionar como humor nessa missão e, mais tarde, como callback em situação de estresse.


## 22. Bifurcacao das rotas — morte de NULL
A exposicao na Deep Web nao define mais Policia vs. Mafia. Ela permanece como variavel de mundo e consequencia.

A bifurcacao ocorre quando `[NICKNAME]`, depois de `Adeus` e do offline de NULL, decide quem usara para elimina-lo:

- **Mikhail** -> Rota da Mafia. A organizacao elimina NULL, desaparece com o corpo/equipamentos e recolhe a investigacao, passando a conhecer a identidade real do protagonista.
- **Assassino independente** -> Rota da Policia. O assassinato gera rastros e uma investigacao propria que converge com os crimes digitais acumulados.

O jogador nao escolhe uma rota em menu. Ele escolhe uma acao dentro do mundo, e a consequencia define silenciosamente a Sessao 6.

A morte de NULL permanece canonica nos dois caminhos.
